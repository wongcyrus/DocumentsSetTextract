...
exports.lambdaHandler = async(event, context) => {
    ...
    const rolateResults = await Promise.all(images.filter(c => c.isInverted).map(c => rotateImage(c.key)));

    console.log(rolateResults);
...
const rotateImage = async key => {
    return new Promise((resolve, reject) => {
        s3.getObject({ Bucket: process.env['ImagesBucket'], Key: key }).promise()
